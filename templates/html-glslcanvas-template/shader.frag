...
void main(void)
{
    vec2 uv = 2.*gl_FragCoord.xy/u_resolution.y-vec2(0.5);
    //uv += kaleidio(uv,20.);
    uv.x += .7*sin(79.*PI*uv.y+u_time);
    uv.y += 0.7*sin(7.*PI*uv.x+u_time*2.);
    vec3 col = vec3(0.0);
    for(float i = 0.; i < 1.0; i += .1){
    col += sin(20.*step(rhombSDF(uv-vec2(-.25+i,0.0-i)),1.));
